...
public interface HomeContract {
    ...
    interface View extends IView {
        void setHomeBanner(List<HomeBanner> data);

        void setHomeArticles(List<Article> data);

        void setHomeArticle(List<Article> data);
    }

    //Model层定义接口,外部只需关心Model返回的数据,无需关心内部细节,即是否使用缓存
    interface Model extends IModel {

        Observable<WanAndroidResponse<List<HomeBanner>>> getHomeBanner();

        Observable<WanAndroidResponse<List<Article>>> getTopArticles();

        Observable<WanAndroidResponse<ArticleInfo>> getHomeArticles(int page);
    }
}
