...
public class HomePresenter extends BasePresenter<HomeContract.Model, HomeContract.View> {
    ...
    public void getHomeBanner() {
        mModel.getHomeBanner()
                ...
                .subscribe(new ErrorHandleSubscriber<WanAndroidResponse<List<HomeBanner>>>(mErrorHandler) {
                    @Override
                    public void onNext(WanAndroidResponse<List<HomeBanner>> homeBanners) {
                        mRootView.setHomeBanner(homeBanners.getData());
                    }
                });
    }
        ...
        Observable.zip(mModel.getTopArticles(), mModel.getHomeArticles(0),
                ...
                .subscribe(new ErrorHandleSubscriber<List<Article>>(mErrorHandler) {
                    ...
                    public void onNext(List<Article> articles) {
                        mRootView.setHomeArticles(articles);
                    }
                });
    ...
    public void getHomeArticle() {
        mModel.getHomeArticles(mPage++)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .compose(RxLifecycleUtils.bindToLifecycle(mRootView))
                .subscribe(new ErrorHandleSubscriber<WanAndroidResponse<ArticleInfo>>(mErrorHandler) {
                    @Override
                    public void onNext(WanAndroidResponse<ArticleInfo> articleInfo) {
                        mRootView.setHomeArticle(articleInfo.getData().getDatas());
                    }
                });
    }
}
