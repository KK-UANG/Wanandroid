...
public class SquarePresenter extends BasePresenter<SquareContract.Model, SquareContract.View> {
    ...
    public void getSquareArticles() {
        mModel.getSquareArticles(0)
                ...
                .subscribe(new ErrorHandleSubscriber<WanAndroidResponse<ArticleInfo>>(mErrorHandler) {
                    ...
                    public void onNext(WanAndroidResponse<ArticleInfo> articleInfo) {
                        mRootView.setSquareArticles(articleInfo.getData().getDatas());
                    }
                });
    }

    public void getSquareArticle() {
        mModel.getSquareArticles(mPage++)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .compose(RxLifecycleUtils.bindToLifecycle(mRootView))
                .subscribe(new ErrorHandleSubscriber<WanAndroidResponse<ArticleInfo>>(mErrorHandler) {
                    @Override
                    public void onNext(WanAndroidResponse<ArticleInfo> articleInfo) {
                        mRootView.setSquareArticle(articleInfo.getData().getDatas());
                    }
                });
    }
}
