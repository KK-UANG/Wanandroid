 ...
 */
@FragmentScope
public class PublicPagerPresenter extends BasePresenter<PublicPagerContract.Model, PublicPagerContract.View> {
    @Inject
    ...
    @Inject
    public PublicPagerPresenter(PublicPagerContract.Model model, PublicPagerContract.View rootView) {
    ...
    public void getWxArticleLists(Integer id) {
        mModel.getWxArticleLists(id, 1)
                ...
                .subscribe(new ErrorHandleSubscriber<WanAndroidResponse<ArticleInfo>>(mErrorHandler) {
                    ...
                    public void onNext(WanAndroidResponse<ArticleInfo> data) {
                        mRootView.setWxArticleLists(data.getData());
                        pager.put(id, 1);
                    }
                });
    }

    public void getProjectArticle(Integer id) {
        Integer page = pager.get(id);
        mModel.getWxArticleLists(id, page + 1)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .compose(RxLifecycleUtils.bindToLifecycle(mRootView))
                .subscribe(new ErrorHandleSubscriber<WanAndroidResponse<ArticleInfo>>(mErrorHandler) {
                    @Override
                    public void onNext(WanAndroidResponse<ArticleInfo> data) {
                        mRootView.setWxArticleList(data.getData());
                        pager.put(id, page + 1);
                    }
                });
    }
